...
AudioPluginInstance* InternalPluginFormat::createInstance (const String& name)
{
	...
	if (name == MidiParameterControl::getIdentifier())  return new MidiParameterControl(MidiParameterControl::getPluginDescription());
...
void InternalPluginFormat::getAllTypes (OwnedArray<PluginDescription>& results)
{
	...
	results.add (new PluginDescription(MidiParameterControl::getPluginDescription()));
}
